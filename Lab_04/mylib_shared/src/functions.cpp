...
__declspec(dllexport) void Logger::INFO(int loglevel, const std::string text, const std::string object_name,const std::string message, const int count){
    if (loglevel < loglevel_){
        return;
    }
    if (loglevel == 2){
        std::string time = current_time();
        std::cout<<"!INFO! "<<time<<" "<<text<<" "<<object_name<<" "<<message<<count<<std::endl;
    }
}
